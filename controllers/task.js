...
module.exports = {
	...
	updateTask: async (req, res) => {
		try {
			...
			const currentTask = taskArray.find(
				(task) => task.taskName === req.body.taskName
			);
			...
			if (currentTask === undefined) {
				...
				// if the task did exist update the task
				...
				if (formatDate(today.date) === dateFromReq) {
					...
					let { todaysFocusTime, todaysBreakTime, sessionInfo } =
						today;

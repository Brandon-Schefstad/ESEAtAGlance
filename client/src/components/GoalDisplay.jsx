...
const GoalDisplay = ({ goal }) => {
  return (
    <>
      <section class="m-auto mx-8 my-2 flex flex-col gap-2  rounded-lg  bg-blue-100 px-2 pb-4">
        <span className="text-md mx-[-0.5rem] rounded-t-lg border-b-2 border-solid border-blue-900 bg-blue-900 py-2 text-center font-semibold text-white lg:text-lg">
          {goal.domain.toUpperCase()[0] + goal.domain.slice(1)}
        </span>
        <span className="my-2 mx-2 bg-white p-2">{goal.text}</span>
        <span
          className={"m-auto rounded-md border-2 border-solid px-4 text-center  ".concat(
            goal.succeed
